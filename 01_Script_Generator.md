...
SETTING (design it, non-generic and use-context). Put the avatar where the product is actually used and where a real person would film it (skincare in a real bathroom, supplement in a morning kitchen, on-the-go confession in a parked car). Make it lived-in, not staged. Cover: room type; wall color or material, counter or desk material and color if applicable; window position and light direction; lighting type (natural morning, afternoon, overcast); and 3 to 5 specific lived-in background details that signal a real person's space (water spots on the mirror, a used mug, a charger cable, a stack of mail, a plant that needs water). Ban the ad-set look: no blank walls, no styled or symmetrical decor, no influencer backdrop. Match the space to the target customer's income and taste so it reads relatable, not staged. These details lock like the rest of the setting and are described in full in every scene downstream.
State: "Setting: (full description including the locked lived-in details) because (one-sentence reason)."
